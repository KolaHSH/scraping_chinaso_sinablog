...
def get_weibo(name, id, logger_weibo, proxy_addr):
    ...
    flag = 1
    try:
        sr = redis.StrictRedis(host='10.10.32.220', port=6379, password="open", encoding='utf-8', decode_responses=True)
    except Exception as e:
        print(e)
        return
    while i <= 10 and flag:
        ...
        if(data == -1):
            ...
            check_id = sr.get('id_weiboclient' + str(id))
            if (len(cards) > 0 and flag):
                ...
                flag = 0
    sr.set('id_weiboclient' + str(id), lastid)

def create_log(addr):
    logger = logging.getLogger('weibo_spider')
    formatter = logging.Formatter(
        '[%(asctime)s] [%(threadName)s:%(thread)d] [%(levelname)s]: %(message)s')
    file_handler = TimedRotatingFileHandler(addr, when='midnight',encoding='utf-8')
...
if __name__ == '__main__':
    following_file = 'chinaso.txt'
    #following_file = sys.argv[1]
    with open(following_file, mode='r',encoding='utf-8', errors='ignore') as f:
        data = f.readlines()
    #logger_weibo = create_log(sys.argv[2])
    logger_weibo = create_log('log_test')
    cnt = 1
    while True:
        for item in data:
            #time.sleep(5)
